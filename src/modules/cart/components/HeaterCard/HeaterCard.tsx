...
export const HeaterCard = (): JSX.Element | null => {
  ...
  let heaterInCart = cart?.items?.find(
    (item) => item.product_title === "Heater"
  )
  const [quantity, setQuantity] = useState<number>(heaterInCart?.quantity ?? 1)
  ...
  useEffect(() => {
    heaterInCart = cart?.items?.find((item) => item.product_title === "Heater")
    setHeater(heaterInCart ?? null)
    setQuantity(heaterInCart?.quantity ?? 1)
  }, [cart])
  ...
  return !heater ? null : (
    <div className="flex h-[190px] items-center gap-5 p-5 rounded-[20px] border border-solid border-[#69727a]">
      <div
        ...
        style={{
          background: `url(${heater?.product?.thumbnail}) no-repeat center center / cover`,
        }}
      />
