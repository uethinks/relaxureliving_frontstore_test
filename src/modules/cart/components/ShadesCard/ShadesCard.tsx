...
const useQuantityManager = (cart: any, updateVariantInfo: any) => {
  ...
  useEffect(() => {
    const shadesInCart = cart?.items?.filter(
      (item: any) => item.product_handle?.includes("shade-screen")
    )
...
const ShadeCardItem = ({
  ...
  <div className="full flex flex-col md:flex-row items-start gap-5 border-t border-white">
    ...
    <div className="flex flex-col w-full h-[160px] items-start justify-between gap-4 relative flex-1 pt-5">
      <div className="flex flex-col lg:flex-row items-start justify-between relative self-stretch w-full flex-[0_0_auto]">
