...
export const GlassDoorCard = (): JSX.Element | null => {
  ...
  let glassDoorInCart = cart?.items?.find(
    (item) => item.product_title === "Frameless Sliding Glass Door"
  )
  const [quantity, setQuantity] = useState<number>(
    glassDoorInCart?.quantity ?? 1
  )
  ...
  const removeProduct = async () => {
    if (glassDoor?.id) {
      await removeVariant(glassDoor?.id)
      await getCart()
    }
  }
  ...
  useEffect(() => {
    glassDoorInCart = cart?.items?.find(
      (item) => item.product_title === "Frameless Sliding Glass Door"
    )
    setGlassDoor(glassDoorInCart ?? null)
    setQuantity(glassDoorInCart?.quantity ?? 1)
  }, [cart])
  ...
  return !glassDoor ? null : (
    <div className="flex h-[190px] items-center gap-5 p-5 rounded-[20px] border border-solid border-[#69727a]">
      <div
        ...
        style={{
          background: `url(${glassDoor?.product?.thumbnail}) no-repeat center center / cover`,
        }}
      />
