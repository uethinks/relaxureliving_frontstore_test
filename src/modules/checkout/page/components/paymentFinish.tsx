...
// 提取成功/失败消息组件
const PaymentMessage = ({ success }: { success: boolean }) => {
...
// 提取地址信息组件
const AddressInfo = ({ order }: { order: HttpTypes.StoreOrder }) => {
  return (
  ...
  return !paymentFinishShow ? (
    ...
    >
      <div className="flex flex-col w-full lg:w-[547px] items-center justify-start gap-5 p-5 relative bg-[#ffffff] rounded-[20px]">
        ...
        <OrderDetails order={order} success={success} />
        <TotalAndButton order={order} onExploreMore={handleExploreMore} />
      </div>
    </div>
  )
}
