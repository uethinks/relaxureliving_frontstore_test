...
const AirwallexPaymentForm: React.FC<AirwallexPaymentFormProps> = ({ 
  ...
  useEffect(() => {
    const loadDropInElement = async () => {
        console.log("loadDropInElement", new Date().toISOString(), "paymentIntentId", paymentIntent.id, "deliveryInfo", deliveryInfo, "NEXT_PUBLIC_AIRWALLEX_ENV", process.env.NEXT_PUBLIC_AIRWALLEX_ENV)
      ...
      try {
        ...
        element?.mount("dropIn-" + paymentIntent.id); // This 'dropIn' id MUST MATCH the id on your empty container created in Step 4
      } catch (error) {
        console.error(error);
      }
    };
    ...
    const domElement = document.getElementById("dropIn-" + paymentIntent.id);
  ...
  return (
    <div style={{ position: 'relative', width: '100%' }}>
       ...
       */}
      <div
        id={"dropIn-" + paymentIntent.id}
        style={{
          ...containerStyle, // Example: container styling can be moved to css
        }}
      />

      <div className="px-4 py-2 mt-3 inline-flex items-center gap-2 text-[12px] text-[#2a261b]">
