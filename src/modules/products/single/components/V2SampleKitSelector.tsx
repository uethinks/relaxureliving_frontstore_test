...
interface V2SampleKitSelectorProps {
...
export const V2SampleKitSelector: React.FC<V2SampleKitSelectorProps> = ({
  sampleKitProduct,
  sampleKitCMSData,
}) => {
  const { addVariant } = useCart()
  const [selectedColor, setSelectedColor] = useState<StoreProductOptionValue>()
  const [selectedHeater, setSelectedHeater] = useState<StoreProductVariant>()
  ...
  // 获取颜色选项
  const heaterColors = sampleKitProduct?.options?.find(
  ...
  useEffect(() => {
    ...
    setSelectedHeater(variants?.[0])
  }, [])

  // 计算价格
  useEffect(() => {
    console.log("selectedHeater: ", selectedHeater)
    setCalculatedPrice(selectedHeater?.calculated_price?.calculated_amount ?? 0)
    setTotalPrice(
      manualQuantity *
        (selectedHeater?.calculated_price?.calculated_amount ?? 0)
    )

    setTotalOriginalPrice(
      manualQuantity * (selectedHeater?.calculated_price?.original_amount ?? 0)
    )
  }, [selectedHeater, manualQuantity])
  ...
  const getVariant = useCallback(() => {
    return sampleKitProduct?.variants
  }, [sampleKitProduct, selectedColor])
          ...
          <div className="mt-8 mb-6">
            ...
            <div className="flex flex-col items-start gap-5">
              <div className="text-sm font-medium text-[#000000] flex justify-between w-full">
                <span> Amount: {manualQuantity}</span>
