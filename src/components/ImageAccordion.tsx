      ...
      <div className="group flex max-md:flex-col justify-center gap-0">
        {items.map((item, index) => {
          ...
          return (
            <article
              key={item.id}
              className={`group/article relative w-full overflow-hidden transition-all duration-300 ease-[cubic-bezier(.5,.85,.25,1.15)] before:absolute before:inset-x-0 before:bottom-0 before:h-1/3 before:bg-gradient-to-t before:from-black/50 before:transition-opacity focus-within:ring focus-within:ring-indigo-300 ${
            ...
            >
              <div className="absolute inset-0 text-white z-10">
                <span className={`absolute inset-x-0 bottom-0 text-lg font-medium p-6 md:px-12 md:py-8 md:whitespace-nowrap md:truncate transition-all duration-200 ease-[cubic-bezier(.5,.85,.25,1.8)] ${
                  isHovered 
              ...
              <Image
                ...
                width={960}
                height={480}
                className="object-cover h-72 md:h-[480px] w-full"
