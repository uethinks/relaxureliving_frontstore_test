...
interface PayTestComponentProps {
  data: PaymentIntentBody;
}
 
const PayTestComponent: React.FC<PayTestComponentProps> = ({ data }) => { 
  useEffect(() => {
    const loadDropInElement = async () => {
        console.log("loadDropInElement", new Date().toISOString())
      try {
        ...
        const { id, client_secret, currency } = data;
    ...
    const onReady = (event: CustomEvent): void => {
      ...
      window.alert("Element is mounted");
    };
  ...
  }, [data]); // This effect should ONLY RUN ONCE as we do not want to reload Airwallex and remount the elements
...
export default PayTestComponent;